...
class _SimpleSegmentationModel(nn.Module):
    def __init__(self, backbone, classifier):
        ...
        self.convseg = nn.Conv2d(256, 2, 1)
        self.convsdm= nn.Conv2d(256, 1, 1)
        self.Tanh = nn.Tanh()
        # print('true')
    ...
    def forward(self, x):
        ...
        x_sdm = self.convsdm(x)
        x_seg = self.convseg(x)
        x_sdm = F.interpolate(x_sdm, size=input_shape, mode='bilinear', align_corners=False)
        x_seg = F.interpolate(x_seg, size=input_shape, mode='bilinear', align_corners=False)
        return self.Tanh(x_sdm), x_seg


class _SimpleSegmentationModel2(nn.Module):
    def __init__(self, backbone, classifier):
        super(_SimpleSegmentationModel2, self).__init__()
        self.backbone = backbone
        self.classifier = classifier
        self.convseg = nn.Conv2d(256, 1, 1)
        # self.convsdm = nn.Conv2d(256, 1, 1)
