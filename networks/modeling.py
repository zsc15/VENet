...
from ._deeplab import DeepLabHead, DeepLabHeadV3Plus, DeepLabV3, DeepLabV32
...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    if backbone_name =='resnet50':
        model = DeepLabV32(backbone, classifier)
    else:
        model = DeepLabV3(backbone, classifier)
    return model
...
class SRNet(nn.Module):
    def __init__(self, n_channels=4, num_classes=1, pretrained_backbone=False):
        super(SRNet, self).__init__()
        self.Snet = deeplabv3plus_resnet50(num_classes, pretrained_backbone)
        self.Rnet = Unet(n_channels, num_classes)
        pretrained_dict = torch.load('best_deeplabv3plus_resnet101_voc_os16.pth', map_location=torch.device('cpu'))
        model_dict = self.Snet.state_dict()
        # print(model_dict)
        # 1. filter out unnecessary keys
        pretrained_dict = {k: v for k, v in pretrained_dict["model_state"].items() if k in model_dict}
        # 2. overwrite entries in the existing state dict
        # torch.nn.init.uniform_(self.Snet.convsdm.weight.data, 0.1, 1.0)
        # torch.nn.init.constant_(self.Snet.convsdm.bias.data, 0.0)
        torch.nn.init.uniform_(self.Snet.convseg.weight.data, 0.1, 1.0)
        torch.nn.init.constant_(self.Snet.convseg.bias.data, 0.0)
        conv_dict = {"convseg.weight": self.Snet.convseg.weight.data, "convseg.bias": self.Snet.convseg.bias.data}
        pretrained_dict.update(conv_dict)
        model_dict.update(pretrained_dict)
        # 3. load the new state dict
        self.Snet.load_state_dict(pretrained_dict)

    def forward(self, x):
        x_seg = self.Snet(x)
        # print(torch.cat((x, x_seg), dim=1).shape)
        x_res = self.Rnet(torch.cat((x, x_seg), dim=1))
        return x_seg, x_res