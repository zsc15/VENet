...
from networks.unet_model import VENet
...
parser.add_argument('--exp', type=str,  default='VENet_nanfang_Lv_npy', help='model_name')
...
if __name__ == "__main__":
    ...
    if 'npy' in args.test_root_path:
        net = VENet(n_classes=1)
        from inference_one_img import calculate_nanfang_metric2 as calculate
    else:
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_ACWE2, c_1, c_2 = Lgv(soft_seg, label_batch.float(), device, args, dim=0, threshold=args.t)
            loss_seg_dice = dice_loss(soft_seg, label_batch)
            loss_ce = ce_loss(x_seg, label_batch.float())### losses.dice_loss(outputs_soft[:labeled_bs, 1, ...], label_batch[:labeled_bs, 1, ...] == 1)
